...
A radio signal is physically a real-valued voltage varying over time. The receiver does not usually hand you the raw RF waveform as a single scalar sample. Instead, it downconverts the signal to baseband and represents each sample as a complex number:

$$
 s(t) = I(t) + jQ(t) = A(t)e^{j\phi(t)}
$$
...
This is where Euler's identity comes in:

$$
 e^{j\phi} = \cos\phi + j\sin\phi
$$

So the “imaginary” part is not a separate mystery quantity. It is a coordinate system that lets you represent both amplitude and phase in a single compact object. In practice, that means one complex sample carries two orthogonal real-valued pieces of information.

Why two numbers instead of one? Because a single real amplitude sample cannot distinguish phase. A phase shift is exactly what modulation schemes like BPSK, QPSK, and QAM use to carry information. Those phase changes show up as rotation in the I/Q plane.

A good mental model is this:
- the sample is a point in the plane,
- the point's x-coordinate is `I`,
- the point's y-coordinate is `Q`.

If you collect many samples and plot them as `Q` versus `I`, you get a constellation plot. That plot is the geometric view of the modulation.

- QPSK uses four valid symbol locations, so a clean QPSK example should look like four clusters.
- 16-QAM uses sixteen valid symbol locations, so a clean 16-QAM example should look like sixteen clusters.
- Noise makes those clusters fuzzy, but the constellation geometry is still the same.

That is the core intuition behind the whole project: the network is not “seeing magic IQ.” It is seeing a time-ordered sequence of complex samples whose geometry encodes the modulation scheme.
...
**How to approach it:**
1. Start with the dataset shape and make sure you know what each axis means:
   - axis 0 = batch,
   - axis 1 = channel (`I` and `Q`),
   - axis 2 = time/sample index.
   The raw input is therefore a sequence of 1024 complex samples, represented as 2 channels per time step.
2. Write the layers, but do not trust the model to be correct by inspection. Add `print(x.shape)` after each layer and run the code once. The goal is not to "make a pretty notebook" — it's to force the shape story into your head.
3. Work out the expected shape on paper first:
   - `Conv1d(2, 64, kernel_size=7)` keeps the batch dimension the same, changes the channel dimension from 2 to 64, and shortens the sequence length by padding/stride conventions.
   - A nonlinearity does not change shape.
   - `MaxPool1d` shrinks the sequence length.
   - The final `flatten()` must produce one vector per sample in the batch.
4. After the conv stack, check exactly where the dimension bug will appear. It is almost always the transition from the conv feature map to the classifier head.
5. Do **not** apply softmax. `CrossEntropyLoss` does it internally.

A concrete walk-through for this exercise:

- Build a toy `nn.Module` with one `Conv1d` layer, then one `ReLU`, then one `MaxPool1d`, then `flatten`, then `Linear`.
- Feed in a small batch and print the shape after every stage.
- Verify that the output of the last conv layer has the shape you expect before flattening.
- Confirm the flattened tensor is `(batch, features)` and not `(batch, sequence, features)`.
- Make the final linear layer output `(batch, 24)` logits.

The reason this exercise matters is that it turns the architecture from a list of names into a data-flow story. If you can explain what each layer does to the `[batch, 2, 1024]` tensor and why that transformation is appropriate for an IQ sequence, you are already answering the interview question in the right way.
